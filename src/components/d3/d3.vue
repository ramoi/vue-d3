...
<script type='text/javascript'>
	...
	export default {
		...
		methods : {
			...
			clear() {
				this.svcElement.selectAll('*').remove();
			}
		},
		data() {
				return {
				source : undefined, 
				configure : undefined,
				svcElement : undefined
			}
		},
		created() {
			this.svcElement = d3.select(`#${this.id}`)
		},
		watch : {
			source : function(  ) {
				let getLgc = function () {
					const _c = $this.configure
					let $this = this

					if( !!_c.axes ) {
						this.svcElement.append('g')
						.attr('class', 'chart')
					...
					let setSeries = (y, svgG, data, getData, getSeriesName, xScale, yScale ) => {
						...
						// .append('circle')
						// .attr('r', 3)
						// .attr('cx', d => xScale(d[$this.configure.x]) )
